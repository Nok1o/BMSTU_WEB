...
func NewRedisSessionRepository(redisCfg *redis2.RedisConfig) *RedisSessionRepository {
...
func (r *RedisSessionRepository) SaveSession(ctx context.Context, userId uuid.UUID, session models.Session) error {
	logger.Info(ctx, "Trying to save session in Redis for userId: %s", userId.String())
...
func (r *RedisSessionRepository) LookupUserSession(ctx context.Context, session models.Session) (uuid.UUID, error) {
	logger.Info(ctx, "Trying to get user in Redis for sessionId: %s", session.SessionId.String())
...
func (r *RedisSessionRepository) IsExists(ctx context.Context, session uuid.UUID) (bool, error) {
	logger.Info(ctx, "Trying to find user in Redis for sessionId: %s", session.String())
	...
	case err != nil:
		logger.Error(ctx, "Redis connection failed: %s", err.Error())
...
func (r *RedisSessionRepository) DeleteSession(ctx context.Context, session string) error {
	logger.Info(ctx, "Trying to delete session in Redis for sessionId: %s", session)
	...
	logger.Info(ctx, "Successfully deleted session in Redis for sessionId: %s", session)
